...
fn flush_high_level(fd: u64) -> Result<u64, IOError> {
...
pub fn flush(fd: u64, _: u64, _: u64, _: u64, _: u64, _: u64) -> u64 {
    // let buf = unsafe {
    //     core::slice::from_raw_parts_mut(VirtAddr::new(buf).as_mut_ptr::<u8>(), len as usize)
    // };

    flush_high_level(fd).unwrap_or_else(|e| (-(e as i64)) as u64)
}
