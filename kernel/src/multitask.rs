...
use spin::{Lazy, Mutex};
use x86_64::{instructions::interrupts, registers::control::{Cr3, Cr3Flags}, structures::paging::{FrameAllocator, Mapper, Page, PageTableFlags, PhysFrame}, VirtAddr};

use crate::{hlt_loop, println, SetupInfo};
...
static TASKS: Lazy<Mutex<Vec<Arc<Mutex<TaskControlBlock>>>>> = Lazy::new(|| {
	Mutex::new(alloc::vec![
		Arc::new_cyclic(|w| Mutex::new(TaskControlBlock { stack_pointer: VirtAddr::zero(), cr3: Cr3::read(), next_task: w.clone() }))
	])
});

static CURRENT_TASK: Lazy<Mutex<Arc<Mutex<TaskControlBlock>>>> = Lazy::new(|| {
	Mutex::new(TASKS.lock()[0].clone())
});
...
unsafe fn task_switch() {
    ...
    *current_arc_guard = next_arc.clone();
    ...
    unsafe {Cr3::write(next_frame, next_flags);}
    ...
    unsafe {__switch_asm(cur_sp_ptr, next_sp_ptr);}
...
pub fn create_task(
    ...
    for page in Page::range(Page::containing_address(stack_bottom), Page::containing_address(stack_top)) {
	...
	for w in words.into_iter().rev() { // Last is first to enter stack
    ...
    let tcb = Arc::new_cyclic(|weak_self| Mutex::new(TaskControlBlock {
