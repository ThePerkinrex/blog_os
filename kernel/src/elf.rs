...
use object::{read::elf::{ElfFile64, FileHeader, ProgramHeader}, LittleEndian};
use x86_64::{structures::paging::{FrameAllocator, Mapper, Page, PageSize, PageTableFlags, Size4KiB}, VirtAddr};

use crate::{println, stack::{self, Stack}, KERNEL_INFO};
...
pub fn load_example_elf() -> LoadedProgram {
	...
	println!("                type      flags     offset      vaddr      paddr     filesz      memsz      align");
	for p in elf.elf_program_headers() {
		...
		println!("{:>20} {:>10?} {:>10x} {:>10x} {:>10x} {:>10x} {:>10x} {:>10x}", alloc::format!("{:?}", p_type), flags, offset, vaddr, p.p_paddr(LittleEndian), filesz, memsz, p.p_align(LittleEndian));
	...
	for ((offset, filesz), (vaddr, memsz), flags) in loads {
		println!("Loading segment at offset {offset:x} (sz: {filesz:x}) to {vaddr:p} (sz: {memsz:x})");
		let pages = Page::<Size4KiB>::range_inclusive(Page::containing_address(vaddr), Page::containing_address(vaddr + memsz));
		for p in pages {
			...
			if let Some(x) = mapped_pages.get(&p) {
				...
				if page_flags != *x {
					println!("WARN Unexpected page flags ({page_flags:?}) different from already mapped ({x:?})")
				}
			}else{

				println!("Mapping {p:?} with flags {page_flags:?}");
				let frame = info.frame_allocator.allocate_frame().expect("A frame");
				unsafe{info.page_table.map_to(p, frame, PageTableFlags::PRESENT | PageTableFlags::WRITABLE | PageTableFlags::USER_ACCESSIBLE, &mut info.frame_allocator)}.unwrap().flush();
		...
		unsafe{core::ptr::copy_nonoverlapping(elf_start.as_ptr::<u8>(), vaddr.as_mut_ptr(), filesz as usize)}
		unsafe{core::ptr::write_bytes((vaddr + filesz).as_mut_ptr::<u8>(), 0, (memsz-filesz) as usize)};

		highest_page = Some(highest_page.map_or(pages.end, |old| if old.start_address() > pages.end.start_address() {old} else{pages.end}))
	}

	let highest_page = highest_page.unwrap();
	let stack_bottom = highest_page.start_address() + Size4KiB::SIZE;
	let stack = unsafe { stack::create_stack_at(stack_bottom, &mut info.page_table, &mut info.frame_allocator, PageTableFlags::USER_ACCESSIBLE) };
